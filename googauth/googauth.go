...
type GoogAuth interface {
	Authenticate(password string) (bool, error)

	ProvisionURI(user string) string
	ProvisionURIWithIssuer(user string, issuer string) string

	checkScratchCodes(code int) bool
	checkHotpCode(code int) bool
	checkTotpCode(t0, code int) bool
}

func NewGoogAuth(secret string) GoogAuth {
	return &otpConfig{
		Secret:     secret,
		WindowSize: 0,
	}
